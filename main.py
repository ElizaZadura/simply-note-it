#!/usr/bin/env python3
"""
Simply Note It - A simple text editor built with Python and tkinter
"""
...
class SimplyNoteIt:
    def __init__(self, root):
        ...
        self.panels = []  # Will store text widgets for each panel
    ...
    def update_title(self):
        ...
        if self.current_file:
            ...
            self.root.title(f"{filename}{status} - Simply Note It")
        else:
            status = "*" if self.text_changed else ""
            self.root.title(f"Untitled{status} - Simply Note It")
    ...
    def new_file(self):
        ...
        if focused_panel:
            try:
                focused_panel.delete(1.0, tk.END)
            except:
                pass
    ...
    def find_text(self):
        ...
        def find():
            ...
            if search_text:
                ...
                start = "1.0"
                while True:
                    pos = self.text_area.search(search_text, start, tk.END)
                    if not pos:
                        break
    ...
    def split_horizontal(self):
        ...
        if not focused_panel:
            focused_panel = self.text_area
        
        # Create new panel frame
        new_frame = ttk.Frame(self.text_frame)
    ...
    def close_split(self):
        ...
        # Remove the focused panel (except the main one)
        if focused_panel != self.text_area and focused_panel in self.panels:
            self.panels.remove(focused_panel)
            try:
                focused_panel.destroy()
            except:
    ...
    def get_focused_panel(self):
        """Get the currently focused text panel"""
        try:
            focused = self.root.focus_get()
            if hasattr(focused, 'get') and focused in self.panels:
                return focused
        except:
            pass
        return None
    ...
    def reorganize_panels(self):
        ...
        # Recreate panels with proper parent
        if len(self.panels) == 1:
            # Single panel
            self.panels[0].pack(in_=self.text_frame, fill=tk.BOTH, expand=True)
        elif len(self.panels) == 2:
            # Two panels side by side
            self.panels[0].pack(in_=self.text_frame, side=tk.LEFT, fill=tk.BOTH, expand=True)
            self.panels[1].pack(in_=self.text_frame, side=tk.RIGHT, fill=tk.BOTH, expand=True)
